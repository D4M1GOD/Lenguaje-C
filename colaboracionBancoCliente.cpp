...
class Cliente {
	...
	float monto;
public:
	Cliente(const char nom[40]); //se inicializara (Constructor)
...
class Banco {
	Cliente cliente1, cliente2, cliente3; //atrinutos
public:
	Banco(); //Constructor
	void operar();
...
Cliente::Cliente(const char nom[40]) //banco llamara a cliente
{
	strcpy(nombre, nom);
	monto = 0;
}
...
Banco::Banco(): cliente1("juan"), cliente2("pedro"), cliente3("luis") //Constructor de banco se inicaliza tres objetos de clase cliente
{
}
